...
namespace Freelance_Project.Test.Services
{
    ...
    public class ProjectProposalServiceTest
    {
        ...
        {
            var proposalId = Guid.NewGuid();
            _proposalRepoMock.Setup(r => r.Get(proposalId)).ReturnsAsync(new Proposal { Id = proposalId, IsActive = true, IsAccepted = true });
            var ex = Assert.ThrowsAsync<AppException>(async () => await _service.RejectProposal(proposalId, Guid.NewGuid()));
